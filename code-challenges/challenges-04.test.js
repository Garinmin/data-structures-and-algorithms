...
const sortByPrice = (arr) => {
  const newArray = arr.sort((a,b)=>(a.price>b.price)?1:-1);
...
const sortNumbersByLength = (arr) => {
  const strArray = arr.map(String);
  const newArray =  strArray.sort((a, b) => a.length - b.length);
...
const sortPeople = (arr) => {
  const newArray = arr.sort((a, b) => a.lastName.localeCompare(b.lastName));
  return newArray;
};

/* let character = characters.filter(character => character.lastName);
  return character;
} */
