...
const updateNumbers = (obj) => {
  ...
  for (let property in obj){
    let newName = property + ': ' + obj[property];
    newObj.push(newName);
  }
  return newObj;
};
...
const totalCharacters = (arr) => {
  let i = arr.length;
  Object.values(arr).forEach(value => {
    if (value.spouse === undefined || value.spouse === null){
      i = i;
    } else {
      if (value.children !== undefined){
        i = i + 1 + value.children.length;
      } else {
        i = i + 1;
      }
    }
